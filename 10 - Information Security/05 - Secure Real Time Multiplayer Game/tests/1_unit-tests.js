...
suite('Unit Tests', () => {
	...
	suite('Collectible class', () => {
		...
		test('Collectible item object contains x and y coordinates and a unique id.', (done) => {
			...
			assert.typeOf(testItem.position.x, 'Number');
			assert.typeOf(testItem.position.y, 'Number');
	...
	suite('Player class', () => {
		...
		test('Player object contains a score, x and y coordinates, and a unique id.', (done) => {
			...
			assert.typeOf(testPlayer.position.x, 'Number');
			assert.typeOf(testPlayer.position.y, 'Number');
		...
		test("movePlayer(str, num) adjusts a player's position.", (done) => {
			...
			testPlayer.move();
			const testPos1 = { x: testPlayer.position.x, y: testPlayer.position.y };
			const expectedPos1 = { x: 105, y: 100 };

			testPlayer.movePlayer('left', 10);
			testPlayer.move();
			const testPos2 = { x: testPlayer.position.x, y: testPlayer.position.y };
